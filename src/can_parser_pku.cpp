...
namespace golfmiddleware{
	...
	private:
		...
		int io_byte_dot_bit_[9][2] = {
				{1,3}, //HONK
				{1,7}, //HIGHLIGHT K16
				{1,5}, //LEFT_SIGNAL_LIGHT K14
				{1,6}, //RIGHT_SIGNAL_LIGHT K15
				{1,0}, //BRAKE_LIGHT K9
				{0,1}, //CONTROLLER
				{0,5}, //REAR_LEFT_SIGNAL_LIGHT, K6
				{0,6}, //REAR_RIGHT_SIGNAL_LIGHT, K7
				{0,7}, //WIDTH_LIGHT
		};
...
namespace golfmiddleware{
	void CanParser::onInit()
	{
		...
		highlight_sub_  = nh_.subscribe<std_msgs::Bool>("/decision/highlight", 1,
														boost::bind(&CanParser::GeneralHandler, this, _1, HIGHLIGHT));
		left_signal_light_sub_ = nh_.subscribe<std_msgs::Bool>("/decision/left_signal_light", 1,
															   boost::bind(&CanParser::GeneralHandler, this, _1, LEFT_SIGNAL_LIGHT));
		right_signal_light_sub_ = nh_.subscribe<std_msgs::Bool>("/decision/right_signal_light",1,
																boost::bind(&CanParser::GeneralHandler, this, _1, RIGHT_SIGNAL_LIGHT));
		brake_light_sub_ = nh_.subscribe<std_msgs::Bool>("/decision/brake_light",1,
														 boost::bind(&CanParser::GeneralHandler, this, _1, BRAKE_LIGHT));
		controller_sub_ = nh_.subscribe<std_msgs::Bool>("/decision/controller",1,
														boost::bind(&CanParser::GeneralHandler, this, _1, CONTROLLER));
		cmd_vel_sub_ = nh_.subscribe("/decision/cmd_vel",1, &CanParser::CmdVelHandler, this);
		brake_sub_ = nh_.subscribe("/decision/brake",1, &CanParser::BrakeHandler, this);
		battery_pub_ = nh_.advertise<std_msgs::Float32>("/battery",1);
		error_state_pub_ = nh_.advertise<std_msgs::UInt8>("/error_state",1);
		angle_pub_ = nh_.advertise<geometry_msgs::Vector3Stamped>("/steering_wheel_angle",1);
		...
		nh_.param<int>("car_max_motor_speed_reverse", car_max_motor_speed_reverse_, 5500);
		...
		speed_frame_.data = {0x18, 0x00, 0x64, 0x05, 0x84, 0x03, 0x00, 0x00};


		// io_frame init end

		max_speed_reciprocal_ = 1.0 / (static_cast<double>(car_max_motor_speed_) / 3000.0 * 6.481481481) * 100.0;
		max_speed_reciprocal_reverse_ = -1.0 / (static_cast<double>(car_max_motor_speed_reverse_) / 3000.0 * 6.481481481) * 100.0;
		{
			std::unique_lock<std::mutex> lock(can_buffer_mutex_);
			can_to_send_buffer_.clear();
		}

		heartbeat_timer_ = nh_.createTimer(ros::Duration(heartbeat_cycle_), &CanParser::HeartBeatCb, this);
		receive_timer_ = nh_.createTimer(ros::Duration(trans_receive_cycle_), &CanParser::ReceiveCb, this);
		transmit_timer_ = nh_.createTimer(ros::Duration(trans_receive_cycle_), &CanParser::TransmitCb, this);

		can_bridge_ptr_ = std::shared_ptr<cantools::CanBridge>(new cantools::CanBridge());
		can_bridge_ptr_->CanOpen();

		initialize_finished_ = true;
	}

	void CanParser::GeneralHandler(std_msgs::BoolConstPtr msg, const IoType& io_type)
	{
	...
	void CanParser::SetBit(const IoType &io_type)
	{
			...
			io_frame_.data[io_byte_dot_bit_[io_type][0]] |= (1 << io_byte_dot_bit_[io_type][1]);
			ComputeCrcCheck();
			//		v{
			//			std::unique_lock<std::mutex> lock(can_buffer_mutex_);
			...
			if (!brake_state_) {
					...
					if (linear_x >= 0)
					{
							...
							} else if ((linear_x - last_linear_x_ < 0) && acc_flag_ != -1)
							{
									acc_flag_ = -1;
									SetBit(BRAKE_LIGHT);
							} else if ((linear_x - last_linear_x_ == 0) && linear_x == 0 && acc_flag_ != -1) {
									acc_flag_ = -1;
									SetBit(BRAKE_LIGHT);
							} else if ((linear_x - last_linear_x_ == 0) && linear_x > 0 && acc_flag_ != 1){
									acc_flag_ = 1;
					...
					if (linear_x > min_tolerance) {
							...
							speed_frame_.data[0] = 0x15;
							speed_frame_.data[1] = static_cast<uint8_t >(std::min((linear_x * max_speed_reciprocal_), 100.0));
							//	ROS_INFO_STREAM("linear speed: " << linear_x);
							//	ROS_INFO_STREAM("parameter is " << max_speed_reciprocal_);
							//ROS_INFO_STREAM("speed_is_" << static_cast<int>(static_cast<uint8_t >(std::min((linear_x * max_speed_reciprocal_), 100.0))));
							//ROS_INFO_STREAM("max_speed_is_" << max_speed_reciprocal_);
							speed_frame_.data[2] = 0x64;
							speed_frame_.data[3] = 0x05;
							speed_frame_.data[4] = static_cast<uint16_t>(std::min(std::max(-angular_z / 67.5 * 900 + 900, 500.0), 1300.0)) & 0xFF;
							speed_frame_.data[5] = static_cast<uint16_t >(std::min(std::max(-angular_z / 67.5 * 900 + 900, 500.0), 1300.0)) >> 8;
							...
							linear_x = (linear_x < -min_walk)? linear_x : (-min_walk);
							speed_frame_.data[0] = 0x16;
							speed_frame_.data[1] = static_cast<uint8_t >(std::min(linear_x * max_speed_reciprocal_reverse_, 100.0));
							speed_frame_.data[2] = 0x64;
							speed_frame_.data[3] = 0x05;
							speed_frame_.data[4] = static_cast<uint16_t >(std::min(std::max(-angular_z / 67.5 * 900 + 900, 500.0), 1300.0)) & 0xFF;
							speed_frame_.data[5] = static_cast<uint16_t >(std::min(std::max(-angular_z / 67.5 * 900 + 900, 500.0), 1300.0)) >> 8;
	...
	void CanParser::ReceiveCb(const ros::TimerEvent &) {
			...
			if (can_bridge_ptr_->Read(to_read_buffer_) > 0) {
					for (auto& it:to_read_buffer_)
					{
							...
							if (it.id == 0x2F2)
							{
									...
									if (battery_cnt++ >= 50) {
											...
											battery_msg.data = static_cast<double>(it.data[5]) / 255.0 * 100.0;
					...
					//ROS_WARN_STREAM("The size of can read buffer is: " << to_read_buffer_.size());
					//TODO: parser
					// for (auto& it: to_read_buffer_)
					// {
